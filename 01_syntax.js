...
// terminal 창에서 node 01_syntax.js으로 실행
// node 01 + tab
...
//그 외: sanke_case(파이썬), CAPITAL_SNAKE_CASE,YELLING_SNAKE_CASE(상수, 리터럴), kebab-case(변수명에는 쓸수없음)

var b; //b라고 하는 변수명을 메모리에 탑재해서 사용하겠다
b = 2; //b에 2를 할당
b = "HELLO";

//var는 여러 문제(호이스팅, 스코프 문제 등)가 있어서 지양
//호이스팅 : 실행문보다 선언문이 뒤에 나와도 에러가 없음(맨처음으로 가서 선언이 되어버림) ->없는 변수를 쓸수 있게 되어버림
//재선언을 해도 전혀 에러없이 지나가버림
var e;
var e; //재선언

//null은 메모리가 의도적으로 비워진 상태로 할당됨
//undefined는 해당된 메모리 공간이 없음
//선언은 되었는데 아무 값도 안주었을 때도 undefined
//특정 객체의 프로퍼티를 못 찾을 때도 undefined
